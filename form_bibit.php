...
$bahan_baku = [];
$sql_bahan_bakubibit = "SELECT * FROM bahan_bakubibit";
$result_bahan_baku = $conn->query($sql_bahan_bakubibit);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Validasi input
    if (empty($_POST['bahan_bakubibit']) || empty($_POST['item_quantity']) || empty($_POST['full_name']) || empty($_POST['shipping_address']) || empty($_POST['city']) || empty($_POST['province']) || empty($_POST['postal_code'])) {
        echo "<script>alert('Harap lengkapi semua data wajib!');</script>";
    } else {
        $bahan_id = $_POST['bahan_bakubibit'];
        $item_quantity = (int)$_POST['item_quantity'];

        // Ambil data bahan baku
        $stmt = $conn->prepare("SELECT * FROM bahan_bakubibit WHERE id = ?");
        ...
        if ($result->num_rows === 0) {
            ...
            $sql_insert = "INSERT INTO orders_bibit (full_name, company_name, email, shipping_address, city, province, postal_code, additional_message, item_name, item_price, item_quantity, total_price, bahan_id) 
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($stmt->execute()) {
                ...
                $summary_sql = "SELECT * FROM orders_bibit WHERE id = ?";
...
<body>
    ...
    <div class="container">
        <form method="POST" action="">
            ...
            <select name="bahan_bakubibit" required>
        ...
        <div class="summary">
            ...
            <form method="POST" action="payment_bibit.php">
