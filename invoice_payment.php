...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['payment_proof'])) {
    ...
    if (!in_array($file_type, $allowed_types)) {
        ...
        if (move_uploaded_file($file['tmp_name'], $target_file)) {
            ...
            // Update data pesanan
            $stmt = $conn->prepare("UPDATE orders SET payment_proof = ?, payment_status = 'Paid', tracking_number = ? WHERE id = ?");
...
$stmt = $conn->prepare("SELECT o.*, b.nama AS item_name, b.harga AS item_price FROM orders o JOIN bahan_baku b ON o.bahan_id = b.id WHERE o.id = ?");
